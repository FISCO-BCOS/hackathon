...
GROUP_NUM = 4
# 领导客户端数量
GROUP_CLIENT_NUM = int(CLIENT_NUM/GROUP_NUM)
...
def preprocess_data(data):
    ...
    droplist = ['V8', 'V13', 'V15', 'V20', 'V21', 'V22', 'V23', 'V24', 'V25', 'V26', 'V27', 'V28', 'Amount', 'Hour']
    data = data.drop(droplist, axis=1)
    x_feature = list(set(x_feature) - {'V8', 'V13', 'V15', 'V20', 'V21', 'V22', 'V23', 'V24', 'V25', 'V26', 'V27',
                                       'V28', 'Amount', 'Hour'})
...
def split_data(path, clients_num):
    ...
    # 训练集划分给多个client，但是测试集并没有划分给多个client
...
X_train, X_test, y_train, y_test = split_data("./data/creditcard.csv", CLIENT_NUM)


# 写一个节点的工作流程
def run_one_node(node_id, nano_id):
    """指定一个node id，并启动一个进程"""
    trained_epoch = -1
    nano_index = int(nano_id.split('_')[-1])
    ...
    try:
        ...
        print("{} of {} initializing....".format(node_id, nano_id))
    ...
    # 训练节点训练
    def local_training():
        print("{} of {} begin training..".format(node_id, nano_id))
        try:
            ...
            W = tf.Variable(ser_W)
            b = tf.Variable(ser_b)
            # 预测结果，用的是一个线性回归模型？？？如果有激活函数就是一层神经网络
            ...
            update_model = {'delta_model': delta_model, 'meta': meta, 'group': nano_index}  # 模型是以字典形式组织起来的W、b列表，样本数量和损失值
    ...
    def local_scoring():
        try:
            ...
            print("{} of {} begin scoring..".format(node_id, nano_id))
            scores = {}
            score_num = 0
            loop_num = 0
            for trainer_id, update in updates.items():
                ...
                delta_model, meta, group = update['delta_model'], update['meta'], update['group']
                ...
                if group == nano_index:
                    ...
                    # print("不属于这个分组，continue")
                    continue
            print("leader node{}打分的个数为：{}".format(node_index, score_num))
            print("leader scores{}".format(serialize(scores)))
            # 将评分上链并且得到凭据receipt
    ...
    def local_aggregation(updates, model, epoch):
        print("node{} of Nano{}本地模型聚合开始...".format(node_index, nano_index))
        agg_model = copy.deepcopy(model)       #
        ...
        for trainer_id, update in updates.items():
            ...
            if aggregation_num >= GROUP_CLIENT_NUM:
                break
            if group == nano_index:
                # print("delta_model:", np.array((delta_model['ser_W'])))
                agg_model['ser_W'] -= (np.array((delta_model['ser_W']))*learning_rate/GROUP_CLIENT_NUM)
                agg_model['ser_b'] -= (np.array((delta_model['ser_b']))*learning_rate/GROUP_CLIENT_NUM)
                agg_meta['avg_cost'] += meta['avg_cost']/GROUP_CLIENT_NUM
                agg_meta['n_samples'] += meta['n_samples']
                aggregation_num += 1
            else:
                continue
        # print("全局聚合模型为:", (model['ser_W'], model['ser_b']))
        # print("本地聚合模型为:", (agg_model['ser_W'], agg_model['ser_b']))
        delta_W = ((np.array((model['ser_W'])) - np.array((agg_model['ser_W']))) / learning_rate).tolist()
        delta_b = ((np.array((model['ser_b'])) - np.array((agg_model['ser_b']))) / learning_rate).tolist()
        ...
        update_model = serialize(update_model)
        # 将本地聚合模型上链
        print("Nano{}聚合的个数为：{}".format(nano_index, aggregation_num))
    ...
    def main_loop():
        try:
            # 注册节点
            receipt = client.sendRawTransactionGetReceipt(to_address, contract_abi, "RegisterNode", [nano_index])
            print("{} of Nano{} registered successfully".format(node_id, nano_index))
            ...
            while True:
                ...
                if global_epoch > MAX_EPOCH:
                    ...
                    break
                # 进行同步，等待所有本地聚合都结束，并完成全局聚合之后再开始下一轮。trained_epoch是每个进程自己维护的变量
        ...
        try:
            ...
            while True:
                ...
                if epoch > test_epoch:
                    ...
                    print("============================================Epoch: {:03}, test_acc: {:.4f}".format(test_epoch, test_acc))
                    test_epoch = epoch
                    # 上传全局模型准确率
                    receipt = client.sendRawTransactionGetReceipt(to_address, contract_abi, "UploadGlobalAccuracy", [str(test_acc)])
                wait()
...
def get_accuracy():
    try:
        client = BcosClient()
        local_accuracy, global_accuracy = client.call(to_address, contract_abi, "QueryAllAccuracy")
        local_accuracy = deserialize(local_accuracy)
        global_accuracy = deserialize(global_accuracy)
        return local_accuracy, global_accuracy, to_address
...
def main():
    ...
    for i in range(CLIENT_NUM):
        nano_id = "nano_" + str(i//GROUP_CLIENT_NUM+1)
        node_id = 'node_{}'.format(len(process_pool))
        p = Process(target=run_one_node, args=(node_id, nano_id))
        p.start()
