...
class Digit extends Base {
  ...
  assetOfOwner(owner) {
    ...
    .then(res=> {
      ...
      for (const record of arr) {
        // console.log(record)
        let [id, ownerAddress, play, url, platform, title] =record.split('^,^')
        list.push({id, ownerAddress, play, url, platform, title})
      }
      return list
    })
  }
  ...
  destroy(id) {
    return this.transact('destory', [id, ''])
  }
  approve(to, id) {
