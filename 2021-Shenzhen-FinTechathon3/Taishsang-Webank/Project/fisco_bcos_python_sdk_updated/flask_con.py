...
def fix_addr(addr):
    if "did:weid" in addr:
        addr = addr.replace("did:weid:3:","")
        return addr
    else:
        wei_addr = "did:weid:3:" + addr
        return wei_addr

@app.route("/tokenURI",methods=["POST","GET"])
def tokenURI():
...
@app.route("/transferTo",methods=["POST"])
def transferTo():
    data = request.get_json()
    addr = fix_addr(data["addr"])
    tokenid = data["tokenid"]
...
@app.route("/tokenOfOwnerByIndex",methods=["POST"])
...
@app.route("/ownerOf",methods=["POST"])
def ownerOf():
    data = request.get_json()
    tokenid = data["tokenid"]
    # index = data["index"]
