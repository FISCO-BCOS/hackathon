...
func (c *BoundContract) transact(opts *TransactOpts, contract *common.Address, input []byte) (*types.Transaction, error) {
	...
	if gasLimit == 0 {
		...
		msg := fiscobcos.CallMsg{GroupId: opts.GroupId, Msg: fiscobcos.CallEthMsg{From: opts.From, To: contract, Value: value, Data: input}}
	...
	if contract == nil {
		rawTx = types.NewContractCreation(nonce, opts.BlockLimit.Uint64(), value, gasLimit, gasPrice, input)
	} else {
		rawTx = types.NewTransactionForFISCOBCOS2(nonce, opts.BlockLimit.Uint64(), c.address, value, gasLimit, gasPrice, input, big.NewInt(1), big.NewInt(1), nil)
	}
	if opts.Signer == nil {
