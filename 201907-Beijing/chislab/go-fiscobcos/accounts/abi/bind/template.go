...
import org.fiscobcos.geth.internal.*;
...
{{range $contract := .Contracts}}
	public class {{.Type}} {
		...
		{{if .InputBin}}
			...
			public static {{.Type}} deploy(TransactOpts auth, FiscoBcosClient client{{range .Constructor.Inputs}}, {{bindtype .Type}} {{.Name}}{{end}}) throws Exception {
		...
		public {{.Type}}(Address address, FiscoBcosClient client) throws Exception {
