...
type Wallet struct {
	...
	deriveChain     fiscobcos.ChainStateReader // Blockchain state reader to discover used account with
	...
	for errc == nil && err == nil {
		...
		for i := 0; i < len(nextAddrs); i++ {
			for empty := false; !empty; {
				// Retrieve the next derived FiscoBcos account
...
// chain state reader.
func (w *Wallet) SelfDerive(bases []accounts.DerivationPath, chain fiscobcos.ChainStateReader) {
	w.lock.Lock()
	defer w.lock.Unlock()
