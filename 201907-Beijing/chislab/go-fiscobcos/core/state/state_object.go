...
// stateObject represents an FiscoBcos account which is being modified.
//
...
type stateObject struct {
	...
	addrHash common.Hash // hash of fiscobcos address of the account
	data     Account
	db       *StateDB
...
func (s *stateObject) empty() bool {
	return s.data.RandomId == 0 && s.data.Balance.Sign() == 0 && bytes.Equal(s.data.CodeHash, emptyCodeHash)
}

// Account is the FiscoBcos consensus representation of accounts.
...
func (self *stateObject) SetNonce(nonce uint64) {
	self.db.journal.append(nonceChange{
		account: &self.address,
		prev:    self.data.RandomId,
	})
	self.setNonce(nonce)
}

func (self *stateObject) setNonce(nonce uint64) {
	self.data.RandomId = nonce
}
...
func (self *stateObject) RandomId() uint64 {
	return self.data.RandomId
}
