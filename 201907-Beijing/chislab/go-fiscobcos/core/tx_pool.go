...
func (pool *TxPool) reset(oldHead, newHead *types.Header) {
	...
	for addr, list := range pool.pending {
		...
		pool.pendingState.SetNonce(addr, txs[len(txs)-1].RandomId()+1)
	}
...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	if pool.currentState.GetNonce(from) > tx.RandomId() {
...
func (pool *TxPool) add(tx *types.Transaction, local bool) (bool, error) {
	...
	if list := pool.pending[from]; list != nil && list.Overlaps(tx) {
		// RandomId already pending, check if required price bump is met
...
func (pool *TxPool) promoteTx(addr common.Address, hash common.Hash, tx *types.Transaction) bool {
	...
	pool.pendingState.SetNonce(addr, tx.RandomId()+1)
...
func (pool *TxPool) Status(hashes []common.Hash) []TxStatus {
	...
	for i, hash := range hashes {
		if tx := pool.all.Get(hash); tx != nil {
			from, _ := types.Sender(pool.signer, tx) // already validated
			if pool.pending[from] != nil && pool.pending[from].txs.items[tx.RandomId()] != nil {
...
func (pool *TxPool) removeTx(hash common.Hash, outofbound bool) {
	...
	if pending := pool.pending[addr]; pending != nil {
		if removed, invalids := pending.Remove(tx); removed {
			...
			if nonce := tx.RandomId(); pool.pendingState.GetNonce(addr) > nonce {
				...
				for pending > pool.config.GlobalSlots && pool.pending[offenders[len(offenders)-2]].Len() > threshold {
					for i := 0; i < len(offenders)-1; i++ {
						...
						for _, tx := range list.Cap(list.Len() - 1) {
							...
							if nonce := tx.RandomId(); pool.pendingState.GetNonce(offenders[i]) > nonce {
