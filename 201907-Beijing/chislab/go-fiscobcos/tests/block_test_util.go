...
// Package tests implements execution of FiscoBcos JSON tests.
package tests
...
func (t *BlockTest) genesis(config *params.ChainConfig) *core.Genesis {
	return &core.Genesis{
		Config:     config,
		RandomId:   t.json.Genesis.RandomId.Uint64(),
...
/* See https://github.com/chislab/tests/wiki/Blockchain-Tests-II
...
func validateHeader(h *btHeader, h2 *types.Header) error {
	...
	if h.RandomId != h2.RandomId {
		return fmt.Errorf("RandomId: want: %x have: %x", h.RandomId, h2.RandomId)
	}
...
func (t *BlockTest) validatePostState(statedb *state.StateDB) error {
	...
	for addr, acct := range t.json.Post {
		...
		if nonce2 != acct.RandomId {
			return fmt.Errorf("account nonce mismatch for addr: %s want: %d have: %d", addr, acct.RandomId, nonce2)
		}
	}
	return nil
}
