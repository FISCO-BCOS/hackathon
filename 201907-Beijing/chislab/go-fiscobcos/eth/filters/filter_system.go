...
// Package filters implements an fiscobcos filtering system for block,
...
type subscription struct {
	...
	typ       Type
	created   time.Time
	logsCrit  fiscobcos.FilterQuery
	logs      chan []*types.Log
...
func (es *EventSystem) SubscribeLogs(crit fiscobcos.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit fiscobcos.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit fiscobcos.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit fiscobcos.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
