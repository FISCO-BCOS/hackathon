...
import (
	...
	"github.com/chislab/go-fiscobcos/eth/fetcher"
...
// NewProtocolManager returns a new FiscoBcos sub protocol manager. The FiscoBcos sub protocol manages peers capable
// with the FiscoBcos network.
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing FiscoBcos peer", "peer", id)

	// Unregister the peer from the downloader and FiscoBcos peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping FiscoBcos protocol")
	...
	log.Info("FiscoBcos protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("FiscoBcos peer connected", "name", p.Name())

	// Execute the FiscoBcos handshake
	var (
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("FiscoBcos handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("FiscoBcos peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("FiscoBcos message handling failed", "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the FiscoBcos sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // FiscoBcos network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
