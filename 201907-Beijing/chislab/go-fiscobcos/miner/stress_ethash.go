...
func main() {
	...
	for _, node := range nodes {
		...
		if err := node.Service(&fiscobcos); err != nil {
			panic(err)
		}
		if err := fiscobcos.StartMining(1); err != nil {
			panic(err)
		}
	}
	time.Sleep(3 * time.Second)
	...
	for {
		...
		var fiscobcos *eth.FiscoBcos
		if err := nodes[index%len(nodes)].Service(&fiscobcos); err != nil {
			panic(err)
		}
		...
		if err := fiscobcos.TxPool().AddLocal(tx); err != nil {
			panic(err)
		}
		nonces[index]++

		// Wait if we're too saturated
		if pend, _ := fiscobcos.TxPool().Stats(); pend > 2048 {
...
func makeMiner(genesis *core.Genesis) (*node.Node, error) {
	// Define the basic configurations for the FiscoBcos node
	...
	// Start the node and configure a full FiscoBcos node on it
