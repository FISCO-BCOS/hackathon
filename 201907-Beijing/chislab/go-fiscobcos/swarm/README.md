...
[https://swarm.fiscobcos.org](https://swarm.fiscobcos.org)

Swarm is a distributed storage platform and content distribution service, a native base layer service of the fiscobcos web3 stack. The primary objective of Swarm is to provide a decentralized and redundant store for dapp code and data as well as block chain and state data. Swarm is also set out to provide various base layer services for web3, including node-to-node messaging, media streaming, decentralised database services and scalable state-channel infrastructure for decentralised service economies.

[![Travis](https://travis-ci.org/fiscobcos/go-fiscobcos.svg?branch=master)](https://travis-ci.org/fiscobcos/go-fiscobcos)
...
Building Swarm requires Go (version 1.10 or later).
    ...
    go get -d github.com/chislab/go-fiscobcos

    go install github.com/chislab/go-fiscobcos/cmd/swarm
...
To run Swarm you need an FiscoBcos account. You can create a new account by running the following command:
...
Once you have specified the password, the output will be the FiscoBcos address representing that account. For example:
...
### FiscoBcos Name Service resolution

The FiscoBcos Name Service is the FiscoBcos equivalent of DNS in the classic web. In order to use ENS to resolve names to Swarm content hashes (e.g. `bzz://theswarm.eth`), `swarm` has to connect to a `geth` instance, which is synced with the FiscoBcos mainnet. This is done using the `--ens-api` flag.

    swarm --bzzaccount <your-account-here> \
          --ens-api '$HOME/.fiscobcos/geth.ipc'
...
You must have your working copy under `$GOPATH/src/github.com/chislab/go-fiscobcos`.

Most likely you will be working from your fork of `go-fiscobcos`, let's say from `github.com/nirname/go-fiscobcos`. Clone or move your fork into the right place:

```
git clone git@github.com:nirname/go-fiscobcos.git $GOPATH/src/github.com/chislab/go-fiscobcos
```
...
```
stateth --rm --grafana-dashboards-folder $GOPATH/src/github.com/chislab/go-fiscobcos/swarm/grafana_dashboards --influxdb-database metrics
```
...
Swarm offers a local HTTP proxy API that Dapps can use to interact with Swarm. The FiscoBcos Foundation is hosting a public gateway, which allows free access so that people can try Swarm without running their own node.
 ...
 * [Code review guidelines](https://github.com/chislab/go-fiscobcos/wiki/Code-Review-Guidelines).
...
The go-fiscobcos library (i.e. all code outside of the `cmd` directory) is licensed under the
[GNU Lesser General Public License v3.0](https://www.gnu.org/licenses/lgpl-3.0.en.html), also
included in our repository in the `COPYING.LESSER` file.

The go-fiscobcos binaries (i.e. all code inside of the `cmd` directory) is licensed under the
