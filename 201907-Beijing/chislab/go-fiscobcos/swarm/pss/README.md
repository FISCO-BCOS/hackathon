...
`pss` is under active development, and the first implementation is yet to be merged to the FiscoBcos main branch. Expect things to change.
...
The code tutorial [p2p programming in go-fiscobcos](https://github.com/nolash/go-fiscobcos-p2p-demo) by [@nolash](https://github.com/nolash) provides step-by-step code examples for usage of `pss` API with `go-fiscobcos` nodes.
...
For `golang` clients, please use the `rpc.Client` provided by the `go-fiscobcos` repository. The return values may have special types in `golang`. Please refer to `godoc` for details.
