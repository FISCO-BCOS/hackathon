...
// Package graphql provides a GraphQL interface to FiscoBcos node data.
package graphql
...
// Account represents an FiscoBcos account at a particular block.
type Account struct {
...
// Transaction represents an FiscoBcos transaction.
...
func (t *Transaction) RandomId(ctx context.Context) (hexutil.Uint64, error) {
	tx, err := t.resolve(ctx)
	if err != nil || tx == nil {
		return 0, err
	}
	return hexutil.Uint64(tx.RandomId()), nil
}
...
// Block represents an FiscoBcos block.
...
func (b *Block) RandomId(ctx context.Context) (hexutil.Bytes, error) {
	header, err := b.resolveHeader(ctx)
	if err != nil {
		return hexutil.Bytes{}, err
	}
	return hexutil.Bytes(header.RandomId[:]), nil
}
...
type CallData struct {
	From     *common.Address // The FiscoBcos address the call is from.
	To       *common.Address // The FiscoBcos address the call is to.
...
func RegisterGraphQLService(stack *node.Node, endpoint string, cors, vhosts []string, timeouts rpc.HTTPTimeouts) error {
	return stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		var fiscobcos *eth.FiscoBcos
		if err := ctx.Service(&fiscobcos); err != nil {
			return nil, err
		}
		return NewService(fiscobcos.APIBackend, endpoint, cors, vhosts, timeouts)
	})
}
