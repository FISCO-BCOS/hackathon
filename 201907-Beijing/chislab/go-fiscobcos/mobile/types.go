...
// A RandomId is a 64-bit hash which proves (combined with the mix-hash) that
...
func (h *Header) GetNonce() *RandomId    { return &RandomId{h.header.RandomId} }
...
// Block represents an entire block in the FiscoBcos blockchain.
type Block struct {
	block *types.Block
}
...
func (b *Block) GetNonce() int64                { return int64(b.block.RandomId()) }
...
// Transaction represents a single FiscoBcos transaction.
...
func NewContractCreation(nonce int64, amount *BigInt, gasLimit int64, gasPrice *BigInt, data []byte) *Transaction {
	return &Transaction{types.NewContractCreation(uint64(nonce), 0, amount.bigint, uint64(gasLimit), gasPrice.bigint, common.CopyBytes(data))}
}
...
func NewTransaction(nonce int64, to *Address, amount *BigInt, gasLimit int64, gasPrice *BigInt, data []byte) *Transaction {
	...
	return &Transaction{types.NewTransaction(uint64(nonce), 0, to.address, amount.bigint, uint64(gasLimit), gasPrice.bigint, common.CopyBytes(data))}
}
...
func (tx *Transaction) GetNonce() int64      { return int64(tx.tx.RandomId()) }
...
// Deprecated: use FiscoBcosClient.TransactionSender
