...
func (opts *TransactOpts) GetNonce() int64      { return opts.opts.RandomId.Int64() }
...
func (opts *TransactOpts) SetNonce(nonce int64)  { opts.opts.RandomId = big.NewInt(nonce) }
...
func DeployContract(opts *TransactOpts, abiJSON string, bytecode []byte, client *FiscoBcosClient, args *Interfaces) (contract *BoundContract, _ error) {
...
func BindContract(address *Address, abiJSON string, client *FiscoBcosClient) (contract *BoundContract, _ error) {
	parsed, err := abi.JSON(strings.NewReader(abiJSON))
	if err != nil {
		return nil, err
	}
