...
// ecrecover extracts the FiscoBcos account address from a signed header.
func ecrecover(header *types.Header, sigcache *lru.ARCCache) (common.Address, error) {
	...
	// Recover the public key and the FiscoBcos address
...
// FiscoBcos testnet following the Ropsten attacks.
type Clique struct {
	...
	signer common.Address // FiscoBcos address of the signing key
...
// Author implements consensus.Engine, returning the FiscoBcos address recovered
...
func (c *Clique) verifyHeader(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
	...
	if !bytes.Equal(header.RandomId[:], nonceAuthVote) && !bytes.Equal(header.RandomId[:], nonceDropVote) {
		return errInvalidVote
	}
	if checkpoint && !bytes.Equal(header.RandomId[:], nonceDropVote) {
...
func (c *Clique) Prepare(chain consensus.ChainReader, header *types.Header) error {
	...
	header.RandomId = types.BlockNonce{}
	...
	if number%c.config.Epoch != 0 {
		...
		if len(addresses) > 0 {
			...
			if c.proposals[header.Coinbase] {
				copy(header.RandomId[:], nonceAuthVote)
			} else {
				copy(header.RandomId[:], nonceDropVote)
			}
		}
		c.lock.RUnlock()
	}
