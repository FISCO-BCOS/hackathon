...
void AirNodeInitializer::init(std::string const& _configFilePath, std::string const& _genesisFile)
{
    ...
    fout << "AirNodeInitializer Start !" << std::endl;
    fout.close();
    ...
    fout.open("test.txt", std::ios_base::app);
    fout << "AirNodeInitializer Start 2 !" << std::endl;
    fout.close();
    ...
    fout.open("test.txt", std::ios_base::app);
    fout << "AirNodeInitializer Start 3 !" << std::endl;
    fout.close();
    ...
    fout.open("test.txt", std::ios_base::app);
    fout << "AirNodeInitializer Start 4 !" << std::endl;
    fout.close();
    ...
    auto raftInitializer = m_nodeInitializer->raftInitializer();
    if (raftInitializer == nullptr) {
        fout.open("test.txt", std::ios_base::app);
        fout << "raftInitializer is nullptr !" << std::endl;
        fout.close();
    }
    
    // auto groupInfo = m_nodeInitializer->raftInitializer()->groupInfo();
    // auto nodeService =
    //     std::make_shared<NodeService>(m_nodeInitializer->ledger(), m_nodeInitializer->scheduler(),
    //         m_nodeInitializer->txPoolInitializer()->txpool(), raftInitializer->raft(),
    //         raftInitializer->blockSync(), m_nodeInitializer->protocolInitializer()->blockFactory());
