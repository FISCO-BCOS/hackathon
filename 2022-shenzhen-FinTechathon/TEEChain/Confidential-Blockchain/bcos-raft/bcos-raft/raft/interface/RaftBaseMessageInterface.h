...
namespace consensus
{
class RaftBaseMessageInterface
{
public:
    using Ptr = std::shared_ptr<RaftBaseMessageInterface>;
    RaftBaseMessageInterface() = default;
    virtual ~RaftBaseMessageInterface() {}
...
inline std::string printRaftMsgInfo(RaftBaseMessageInterface::Ptr _raftMsg)
{
    std::ostringstream stringstream;
    stringstream << LOG_KV("reqHash", _raftMsg->hash().abridged())
                 << LOG_KV("reqIndex", _raftMsg->index()) << LOG_KV("reqV", _raftMsg->view())
                 << LOG_KV("fromIdx", _raftMsg->generatedFrom());
