...
const std::string GROUPS_FIELD_NAME = "groups";            //本地聚合模型
const std::string VALUE_FIELD = "value";                             // value field

// 定义类中所有的接口
const char* const REGISTER_NODE = "RegisterNode(int256)";                         //节点注册
    ...
    Address const& _origin, Address const&)
{
    ...
    */
    if(func == name2Selector[REGISTER_NODE]){
        ...
        // 给节点进行分组
        std::string groups_str = GetVariable(table, _origin, callResult, GROUPS_FIELD_NAME);
        Pair groups = json::parse(groups_str);
        groups[_origin_str] = to_json_string(gp);
        UpdateVariable(table, _origin, callResult, GROUPS_FIELD_NAME, groups);
        ...
        if(roles.find(_origin_str)==roles.end()){
            ...
            if(roles.size() == CLIENT_NUM){
                int i = 1;
                bool groups_leader[COMM_COUNT] = {false};
                for(auto & client : roles){
                    if(i > COMM_COUNT)
                        break;
                    for(int j=1; j<=COMM_COUNT; j++){
                        if((groups[client.first]==j)&&(groups_leader[j-1]==false)){
                            client.second = "comm";
                            i++;
                            groups_leader[j-1]=true;
                            break;
                        }
                    }
                }
...
#if OUTPUT
        std::clog<<"注册节点接口中，角色为，分组为："<<roles[_origin_str]<<groups[_origin_str]<<std::endl;
#endif
    /*
    查询全局状态函数接口
    */
...
//#if OUTPUT
//        std::clog<<_origin<<"进入上传本地更新接口"<<std::endl;
//#endif
        // 获取参数
        std::string update;
        s256 ep;
...
//#if OUTPUT
//        std::clog<<"1本地更新模型为"<<update<<std::endl;
//#endif
        // 如果本地更新已经存在
        if(local_updates.find(_origin_str)!=local_updates.end()){
            #if OUTPUT
                    std::clog<<"本地更新已经存在"<<_origin_str<<std::endl;
            #endif
            return callResult;
        }
        ...
        if(update_count >= NEEDED_UPDATE_COUNT){
            #if OUTPUT
                        std::clog<<"模型数已够，本地模型更新不再收集，update count为"<<update_count<<std::endl;
            #endif
            return callResult;
        }
        // 更新次数+1
...
#if OUTPUT
        std::clog<<_origin<<"进入了上传本地聚合的接口"<<update<<ep<<std::endl;
#endif
#if OUTPUT
        std::clog<<"test:"<<ep+1<<std::endl;
#endif

        // 1. if ep == epoch    epoch是通过查表得到的，ep是委员调用智能合约传进来的
//        std::string epoch_str = GetVariable(table, _origin, callResult, EPOCH_FIELD_NAME);
////#if OUTPUT
////        std::clog<<"epoch_str"<<epoch_str<<std::endl;
////#endif
//        int epoch = json::parse(epoch_str);
//#if OUTPUT
//        std::clog<<"epoch"<<epoch<<std::endl;
//#endif
//        if(ep!=epoch)
//            #if OUTPUT
//                    std::clog<<_origin<<"轮数不对，返回空值"<<update<<epoch<<std::endl;
//            #endif
//            return callResult;
//
//        // 2. 如果客户端是训练节点
//    	std::string roles_str = GetVariable(table, _origin, callResult, ROLES_FIELD_NAME);
////#if OUTPUT
////        std::clog<<"roles_str"<<roles_str<<std::endl;
////#endif
//        Pair roles = json::parse(roles_str);
//#if OUTPUT
//        std::clog<<"roles"<<roles<<std::endl;
//#endif
//        if(roles.find(_origin_str)==roles.end()||roles[_origin_str]=="trainer")
//            #if OUTPUT
//                    std::clog<<_origin<<"角色不对，返回空值"<<update<<epoch<<std::endl;
//            #endif
//            return callResult;
//
//#if OUTPUT
//        std::clog<<_origin<<"获得本地聚合更新"<<std::endl;
//#endif
//
        // 3. 获取本地打分并更新，再插入表中。本地打分是一个列表，存储的是这个委员对所有训练节点的打分
#if OUTPUT
        std::clog<<"update:"<<update<<std::endl;
#endif
        std::string local_aggregation_str = GetVariable(table, _origin, callResult, LOCAL_AGGREGATION_FIELD_NAME);
#if OUTPUT
        std::clog<<_origin<<"local_aggregation_str："<<local_aggregation_str<<std::endl;
#endif
        Pair local_aggregation = json::parse(local_aggregation_str);
        local_aggregation[_origin_str] = update;
#if OUTPUT
        std::clog<<_origin<<"当前本地聚合更新为："<<local_aggregation[_origin_str]<<std::endl;
#endif
...
#if OUTPUT
        std::clog<<_origin<<"当前本地聚合次数为："<<aggregation_count<<std::endl;
#endif

#if OUTPUT
        std::clog<<_origin<<"准备开始全局聚合，本地聚合模型、轮数为："<<std::endl;
#endif
...
void CommitteePrecompiled::InitGlobalModel(Table::Ptr table, Address const& _origin, PrecompiledExecResult::Ptr callResult){
    ...
    Pair groups;
    InsertVariable(table, _origin, callResult, GROUPS_FIELD_NAME, groups);
}
...
void CommitteePrecompiled::GlobeAggregate(Table::Ptr table, Address const& _origin, PrecompiledExecResult::Ptr callResult) {
...
#if OUTPUT
        std::clog<<"comm_scores："<<comm_scores[_origin_str]<<std::endl;
#endif

    // 0. 取每个客户端的评分
    Scores scores;  //这里的scores是一个字符串到浮点数的映射，存放的是最后取完中位数的值

    for(auto & it1 : comm_scores){  //所以comm_scores是多个委员会的打分
	Scores trainer_scores = json::parse(it1.second);
        for(auto & it2 : trainer_scores){
            scores[it2.first] = it2.second;
	    #if OUTPUT
		    std::clog<<"scores[it2.first] :"<<scores[it2.first]<<std::endl;
	    #endif
        }
    #if OUTPUT
            std::clog<<"it1.first :"<<it1.first<<std::endl;
    #endif
    #if OUTPUT
            std::clog<<"it1.second :"<<it1.second<<std::endl;
    #endif

	}
    for(auto & it : scores){
	#if OUTPUT
        	std::clog<<"scores[it.first]："<<scores[it.first]<<std::endl;
	#endif
	}
...
#if OUTPUT
        std::clog<<"scores_vec.size()："<<scores_vec.size()<<std::endl;
#endif
#if OUTPUT
      	std::clog<<"scores_vec[0].second："<<scores_vec[0].first<<" "<<scores_vec[0].second<<std::endl;
#endif
    ...
    Pair groups = GetVariable(table, _origin, callResult, GROUPS_FIELD_NAME);
    ...
    for (auto & it : local_aggregation){
	    ...
	    #if OUTPUT
        	std::clog<<"update_str："<<update_str<<std::endl;
	    #endif
            LocalUpdate update = json::parse(update_str);
	    #if OUTPUT
        	std::clog<<"total_update.meta.n_samples："<<total_update.meta.n_samples <<std::endl;
	    #endif
	    #if OUTPUT
        	std::clog<<"total_update.meta.avg_cost："<<total_update.meta.avg_cost <<std::endl;
	    #endif
	    #if OUTPUT
        	std::clog<<"update.meta.n_samples："<<update.meta.n_samples<<std::endl;
	    #endif
	    #if OUTPUT
        	std::clog<<"update.meta.avg_cost："<<update.meta.avg_cost<<std::endl;
	    #endif
	    ...
	    #if OUTPUT
        	std::clog<<"total_update.delta_model.ser_W[i][j]："<<total_update.delta_model.ser_W[i][j]<<std::endl;
	    #endif
                }
            }
    ...
    total_update.meta.avg_cost /= (float) COMM_COUNT;
	...
	#if OUTPUT
 	std::clog<<"global_model.ser_W[i][j]："<<global_model.ser_W[i][j]		<<std::endl;
	#endif
        }
    }
    for (int i = 0; i < n_class; i++) {
    ...
    bool groups_leader[COMM_COUNT] = {false};
    int leader_num = 0;
    for(int k=0; k<NEEDED_UPDATE_COUNT; k++){        //这个for循环是为了选出COMM_COUNT个leader
        if(leader_num>=4){
            break;
        }
        auto trainer = scores_vec[k].first;
        for(int j=1; j<=COMM_COUNT; j++){
            if((groups[trainer]==j)&&(groups_leader[j-1]==false)){      //这个for循环是为了让每个训练者都遍历到各个组别
                roles[trainer] = "comm";
                groups_leader[j-1]=true;
                leader_num++;
                break;
