<template>
  <div id="loan-application">
    ...
    <el-table :data="tableData" ref="table" height="70vh" style="width: 100%;">
      ...
      <el-table-column align="center" prop="name" label="姓名" width="260"></el-table-column>
      <el-table-column align="center" prop="identity_card" label="身份证" width="300"></el-table-column>
      <el-table-column align="center" prop="" label="还款能力" width="250"></el-table-column>
      <el-table-column align="center" label="操作">
        <template v-slot="{row}">
          <el-button size="mini" type="primary" @click="query(row)">详情</el-button>
          <el-button size="mini" type="warning" @click="evaluate(row)">还款评估</el-button>
          <el-button size="mini" type="danger" @click="reject(row)">拒绝</el-button>
          <el-button size="mini" type="success" @click="pass(row)">通过</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import {endList, endReject, endPass, endQuery} from '@/api/api'
...
export default {
  ...
  methods: {
    getList() {
      endList(this.firstQuery).then(res => {
        let {code} = res.data;
        if (code === 20000) {
    ...
    reject(row) {
      this.$confirm('是否拒绝充电桩？', '充电桩审批-终审', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        endReject({id: row.id}).then(res => {
          let {code} = res.data;
          if (code === 20000) {
            this.getList();
            this.$message({
    ...
    pass(row) {
      this.$confirm('是否通过充电桩？', '充电桩审批-终审', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        endPass({id: row.id}).then(res => {
          let {code} = res.data;
          if (code === 20000) {
            this.getList();
            this.$message({
