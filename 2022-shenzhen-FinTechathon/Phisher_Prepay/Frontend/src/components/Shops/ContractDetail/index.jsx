...
export default function ContractDetail() {
    ...
    const handleClick= async ()=>{
      ...
      let msg={
        ...
        'balance':parseInt(values.bookMoney),
        'text':values.otherItems
      }
      for(const key in msg){
        formData.append(key,msg[key])
      }
      // await axios.post('http://1.12.238.107:8080/card/create',{
      //   userId:values.userName,
      //   shopId:values.shopName,
      //   balance:parseInt(values.bookMoney),
      //   text:values.otherItems
      ...
      await axios({
        method:'POST',
        url:'http://1.12.238.107:8080/card/create',
        data:formData
      })
      .then((response)=>{
      ...
      // await axios.get('http://1.12.238.107:8080/init',{})
      // .then((response)=>{
      //   console.log(response)
      //   Toast.show('提交成功，等待商家确认合同！')
      // }).catch((error)=>{
  ...
  return (
    <div>
      ...
      footer={
        ...
        <Form.Item name='userTel' label='甲方(消费者)联系方式' rules={[{required:true}]}>
            <Input placeholder='请输入联系方式'/>
        </Form.Item>
        <Form.Item name='shopName' label='乙方(商家)姓名' rules={[{required:true}]}>
            <Input placeholder='请输入姓名'/>
        </Form.Item>
        <Form.Item name='shopTel' label='乙方(商家)联系方式' rules={[{required:true}]}>
            <Input placeholder='请输入联系方式'/>
        </Form.Item>
        <Form.Item>
